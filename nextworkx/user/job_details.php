    ...
    <div class="job-main">
        ...
        <div class="related-jobs">
            ...
            <div class="related-job-card">
                <div class="job-left">
                    <span class="posted-time"><?= date('d M Y', strtotime($job['posted_on'])) ?></span>
                    <div class="job-header">
                        <img src="../assets/icons/company-logo.png" alt="Company Logo" class="company-logo">
                        <div>
                            <h4 class="job-title"><?= htmlspecialchars($job['job_title']) ?></h4>
                            <p class="company-name"><?= htmlspecialchars($job['job_role']) ?></p>
                        </div>
                    </div>
                    <div class="job-info">
                        <span><i class="fas fa-briefcase"></i> <?= htmlspecialchars($job['job_role']) ?></span>
                        <span><i class="fas fa-clock"></i> <?= htmlspecialchars($job['job_level']) ?></span>
                        <span><i class="fas fa-wallet"></i> <?= htmlspecialchars($job['currency']) ?> <?= number_format($job['min_salary']) ?> - <?= number_format($job['max_salary']) ?></span>
                        <span><i class="fas fa-map-marker-alt"></i> <?= htmlspecialchars($job['city']) ?>, <?= htmlspecialchars($job['country']) ?></span>
                    </div>
                </div>
                <div class="job-right">
                    <a href="job_details.php?job_id=<?= $job['job_id'] ?>" class="job-details-btn">Job Details</a>
                </div>
            </div>
        </div>
